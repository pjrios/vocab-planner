// PolyDash - God Mode Edition v2.0 - UPDATED 2025-11-28
...
class hero {
  ...
  move(gridInstance) {
    ...
    if (deadContactElementCenter.length > 0 && !godMode) {
      if (floorContactElementCenter.length === 0) {
        this.isDead = true;
      } else {
    ...
    if (this.body.center.y < 0 && !godMode) {
...
class drawing {
  ...
  drawBackGround() {
    ...
    if (!this.backgroundImageCity || !this.backgroundImageCity.width) {
      // Draw a simple gradient background instead
      ...
      const gradient = this.ctxBack.createLinearGradient(0, 0, 0, this.ctxBack.canvas.height);
      const offset = (this.backGroundTimeScroll / 100) % 1;
      gradient.addColorStop(0, '#1a1a2e');
      gradient.addColorStop(0.5 + offset * 0.2, '#16213e');
      gradient.addColorStop(1, '#0f3460');

      this.ctxBack.fillStyle = gradient;
      this.ctxBack.fillRect(0, 0, this.ctxBack.canvas.width, this.ctxBack.canvas.height);

      this.backGroundTimeScroll += frameTimeDiff.dt * this.backgroundSpeed;
      return;
    }

    // Original image-based background code
    let ratio =
...
function game() {
  ...
  frameTimeDiff.dt = ((Date.now() - frameTimeDiff.lastTime) / 1000) * speedMultiplier;
  frameTimeDiff.lastTime = Date.now();
  ...
  if (!heroInstance.isDead && !heroInstance.haveFinished) {
    ...
    if (godMode) {
      ...
      drawingInstance.ctx.shadowColor = 'rgba(255, 215, 0, 1)';
      drawingInstance.ctx.shadowBlur = 20;
      drawingInstance.ctx.font = '40px Orbitron';
      drawingInstance.ctx.fillStyle = 'rgba(255, 215, 0, 0.9)';
      drawingInstance.ctx.textAlign = 'center';
      drawingInstance.ctx.fillText('GOD MODE', drawingInstance.width / 2, 50);
    ...
    if (speedMultiplier !== 1.0) {
      drawingInstance.ctx.save();
      drawingInstance.ctx.font = '20px Orbitron';
      drawingInstance.ctx.fillStyle = 'rgba(255, 255, 255, 0.7)';
      drawingInstance.ctx.textAlign = 'right';
      drawingInstance.ctx.fillText('Speed: ' + speedMultiplier.toFixed(2) + 'x', drawingInstance.width - 20, 30);
      drawingInstance.ctx.restore();
    }
...
function keyEventHandler(event) {
  /*  manage what happen when the gamer press a key */
  ...
  if (event.code === 'KeyG' && event.type === 'keydown') {
    ...
    alert('God Mode: ' + (godMode ? 'ON - You are invincible!' : 'OFF'));
  }

  // LEVEL SKIP (N key)
  if (event.code === 'KeyN' && event.type === 'keydown' && heroInstance.hasStarted) {
    // Teleport to near the end
    let endX = 0;
    gridInstance.grid.forEach((col, idx) => {
      if (col) {
        col.forEach(element => {
          if (element instanceof ending) {
            endX = idx - 5; // 5 units before ending
          }
        });
      }
    });
    if (endX > 0) {
      let newCenter = new point(endX, heroInstance.body.center.y);
      let translateVector = new vector(heroInstance.body.center, newCenter);
      heroInstance.translate(translateVector);
      console.log('Teleported to end');
    }
  }

  // SPEED CONTROL (+ and - keys)
  if (event.code === 'Equal' && event.type === 'keydown') { // + key
    speedMultiplier = Math.min(speedMultiplier + 0.25, 3.0);
    console.log('Speed:', speedMultiplier + 'x');
  }
  if (event.code === 'Minus' && event.type === 'keydown') { // - key
    speedMultiplier = Math.max(speedMultiplier - 0.25, 0.25);
    console.log('Speed:', speedMultiplier + 'x');
  }
